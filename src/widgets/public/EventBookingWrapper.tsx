...
import { Loader2, ArrowLeft, Ticket, Calendar, Clock, MapPin } from "lucide-react";
    ...
    const fetchEvent = async () => {
      try {
        ...
        setEvent({
          ...
          startTime,
          endTime,
          venue: data.locationType === "offline" ? data.venueAddress : "Online Event",
          location: data.locationType === "online" ? data.onlineLink : data.venueAddress,
          price: data.price ?? 0,
        ...
        if (data.ticketTypes && data.ticketTypes.length > 0) {
            ...
            updatePrice(0, 1, data.ticketTypes);
        } else {
    ...
    <div className="min-h-screen bg-background relative selection:bg-primary/10">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-black/80 backdrop-blur-md border-b border-black/5">
          <div className="max-w-2xl mx-auto px-6 h-16 flex items-center justify-between">
            <Link href={`/events/${slugId}`} className="inline-flex items-center gap-2 text-sm font-medium text-muted hover:text-foreground transition-colors">
                <ArrowLeft className="w-4 h-4" /> <span className="hidden sm:inline">Cancel Booking</span>
                <span className="sm:hidden">Cancel</span>
            </Link>
            <div className="text-sm font-bold text-foreground truncate max-w-[200px]">{event.name}</div>
          </div>
      </div>
          ...
          <div className="max-w-xl mx-auto space-y-12">
              ...
              <div className="flex gap-6 items-start animate-fade-in-up">
                  ...
                  <div className="space-y-1">
                      <h1 className="text-2xl font-black text-foreground leading-tight">{event.name}</h1>
              ...
              {event.ticketTypes && event.ticketTypes.length > 0 && (
                  <section className="space-y-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                      <h3 className="text-sm font-bold uppercase tracking-wider text-muted">Select Ticket</h3>
                      <div className="space-y-3">
                          {event.ticketTypes.map((ticket, idx) => (
                              ...
                              >
                                  <div>
                                      ...
                                      <p className="text-sm text-muted">{ticket.price > 0 ? `₹${ticket.price}` : "Free"}</p>
                                  </div>
                                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                                      selectedTicketIndex === idx ? 'border-primary' : 'border-black/20'
                                  }`}>
                                      {selectedTicketIndex === idx && <div className="w-2.5 h-2.5 bg-primary rounded-full" />}
                                  </div>
                              </div>
                          ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between py-2">
                          ...
                          <div className="flex items-center gap-3 bg-surface-1 rounded-lg p-1 border border-black/5">
                              ...
                              >-</button>
                              <span className="font-bold w-4 text-center text-sm">{ticketQuantity}</span>
                              <button 
                                onClick={() => handleQuantityChange(ticketQuantity + 1)}
              ...
              <section className="space-y-5 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-muted">Your Details</h3>
